...
round-2/round-5 verdicts; fieldstone family APPROVED at E27 ("that is
a thing of beauty") — pure drystone: crack-network tessellation,
sphere-morph stones with bed/head overlaps, NO mortar core, rough
stone-chip hearting visible in the gaps.  Renders:
...
|---|---|---|
| R1 | DB-compatible by default, configurable | Defaults: `height_mm=29.6` (the official VISIBLE wall height: DB wall pieces top out 33.1 mm above the datum — 49.7 bbox − 16.6 tall base — minus a typical ~3.5 seat; measured 2026-07-05, §1b of commercial-sets-analysis), `thickness_mm=7.0`, outer face flush to the tile edge; every dimension is a parameter; tall ≈ 68.8; OL scale runs through the same scale machinery as everything else |
...
Region(id='ground', selector=FloodFill(0.5, 0.5), layers=[
    ...
    CutStoneWall(                    # direct tile layer, like FacetedStones
        ...
        height_mm=29.6,              # official visible height; 68.8 = tall
        seed=7,
    ),
...
| E25 | **Perched top rocks + bed overlap** (Shawn: remove mortar between head stones — rocks just sitting on top; +20 % horizontal cracks; why are end rocks sheared?; more face-protrusion variance; rounder top/bottom with overlap like the head joints): (1) top course = perched rocks — the core stops just BELOW the top-course bed (`_core_boxes` override via `_cap_z0`; rubble clipped under it too), so gaps between cap stones look onto the stones beneath, never mortar; the cap stones are attached by the bed overlap pressed up from the course below.  (2) Beds get the E24 overlap treatment: stones dilate vertically U(0.35, 0.80) mm past their bed lines (cap 0.22× height; `is_top` exempt, bottom course keeps its seat), fusing courses into pressed contacts.  (3) Courses 2.2–5.2 (−20 %).  (4) End-rock shearing diagnosed: texture displacement pushed end-stone surfaces past the tile boundary and `_clip_to_tile` planed them flat — 'face' ends now recede 0.7 mm (`_END_MARGIN_MM`), leaving full rounded stones at tile edges.  (5) Proud faces U(0.10, 0.70) with a 15 % deep tail to 1.10 (`_PROUD_*`).  (6) `_BED_FLAT_EXP` 0.40–0.65 (rounder beds — safe now that beds overlap).  Pinch nudge made asymmetric (0.15/0.30 µm) + iterated — parallel-normal pairs moved in lockstep and stayed coincident.  Watertight (0 open, 0 non-manifold), red 0.0000 all five views | Shawn: "ok awesome" |
| E26 | **No mortar at all** (Shawn: "for the fieldstone wall, don't put mortar in them"): `_core_boxes` returns `[]` — the drystone wall is stones all the way through.  Face stones interpenetrate at their bed/head overlaps, the rubble hearting fills the interior, so the union stays one connected watertight solid and every crack is floored by deeper stone with no core plane anywhere behind it.  (E25 had already dropped the core below the top course; this removes it everywhere.  The E10 red-mortar acceptance is now satisfied by construction — there is no mortar to show.)  Watertight, red 0.0000 all five views | Shawn: "I think I still see mortar" — the RUBBLE read as mortar |
| E27 | **Rough rubble chinking** (Shawn: "smooth rubbles no good.  should be lots of little rocks --- very rough"): what read as mortar in E26 was the hearting itself — big smooth lumps 1.6 mm behind the face showed bright featureless mid-faces through the head-joint junction holes (diagnosed with a rubble-red variant of the diag, now `wall_mortar_diag.py --rubble`).  `rubble_stone` (stone/shape.py) reworked: crisp un-rounded facets, direction jitter 0.12 → 0.30, lump 0.90–1.0 → 0.70–1.05, 10–14 support points — a sharp little chip.  Hearting packing: foot 8.5–11 → 3.5–5.5, spacing 4.2 → 2.8, setback 1.6 → 1.3 (~990 shards vs ~100 lumps) — gaps now show densely packed angular stone chips, the references’ pinning-stone read.  Watertight, core-red 0.0000 (trivially — no core exists) | Shawn: **"that is a thing of beauty"** ✓ |
| E57 | **A floor is a wall lying on its side** (Shawn: "make wall and floor generation use the same code, as the floor should just be a wall laying on its side"): the chassis gains `laid_flat` — the wall's HEIGHT axis becomes the second plan axis (courses = pavement rows, bays = slabs, bond = row stagger) and its THICKNESS becomes the pavement depth with the OUTER face up (face proudness = per-stone pavement height; undersides overshoot the datum and one clip grounds them).  The mortar core becomes the under-pavement sheet, hearting becomes joint chinking, `_seat_z` drops the soil to the 0.15 film (the floor IS the terrain).  `StoneFloor` reimplemented as `CutStoneWall(laid_flat=True)` pinned to the square grid (exact-pitch courses/bays, `min_bond=0`, flush boundary pulls, missing/spall/crack overrides à la BrickWall) — the old 160-line grid stamper deleted.  Payoff: `FieldstoneWall(laid_flat=True)` IS a drystone flagstone floor (scene `walls-e12-drystone-floor.tile.py`, demo `floor-drystone`) | pending Shawn |
| E28 | **Brick family (family 2)** — `walls/brick.py: BrickWall(CutStoneWall)`, almost pure configuration on the chassis (the stage-4 refactor payoff): course 2.5–3.0, bay 5.0–6.6, joint 0.5, reveal 1.0, min-bond 2.0, worn finish (chip 0.25, roundover 0.35, relief 0.08 @ 1.2–4.0).  Two kernel tweaks give the brick-worn.jpg reads: `missing_prob` cells place NO unit (the chassis None-guard leaves a dark socket floored by the recessed mortar core — never bottom/top/quoins/ends) and `spall_prob` units re-roll with a 0.35×min-dimension chip budget (broken crumbling brick).  Scene `walls-e8-brick.tile.py` (same corner plan as e6 for comparison), demo set entry `wall-brick`.  Watertight | pending Shawn |
| E29 | **Fieldstone vertical coping** (`coping='vertical'`, refs 02/05): regular courses stop 6.5 mm short of H; a coping course of thin stones ON EDGE (2.2–3.6 mm wide) fills the band.  Same complementarity trick as the beds: each coping joint's LEAN (±0.30 dt/dz) is a shared line both flanking stones evaluate, so the course leans together; per-stone top drop ≤1.1 mm gives the ragged coping line; owning segments start/end with a wider corner slab (T + 0.5–1.5) so no sliver coping quoins.  The sphere-morph flatten axis swaps z→t for coping cells (their flat "beds" are the SIDES); they press sideways (0.15–0.35) and down into the course below (0.30–0.60), which is un-topped so its bed wobble at Hc is shared with the coping bottoms.  Default stays the approved perched rocks | pending Shawn |
| E30 | **Cut-stone crenellations** (`crenellated=True`, city-wall archetype): the crenel floor lands on the course boundary nearest H − crenel_depth (11 mm ≈ two courses of merlon); parapet cells survive only inside merlon intervals (8–11 mm, crenels 5.5–7.5, widths scaled to fit each segment exactly, merlons forced at segment ends so every corner is a merlon), re-cut edges become textured 'face' flanks.  Core: base slab stops rv below the crenel floor (block tops, not mortar) + a mini-core per merlon overlapping down into it (merlon joints stay backed).  Chassis-level, so any family can be crenellated | pending Shawn |
| E31 | **Ruin states** (`ruin=0..1`, chassis-level; hadrians-coursed-rubble.jpg): the top breaks along a slow per-segment height envelope (2 sines, wl 45–110 mm — LONGER than a segment; short wavelengths shred small-stone families into fence pickets), drop = ruin × H × 0.20–0.80; straddling blocks survive at P=0.5 (ragged steps); nothing reads as a dressed cap.  The rubble HEARTING moved to the chassis (`_hearting_parts` + `_heart_cap` hook): drystone families build it always (`hearting=True`), mortared families get it on ruin — the mortar core stays below the lowest break so the exposed band shows packed rubble, never a mortar plane.  Shed shards (`_foot_rubble`) collect at both feet, embedded 40 %.  Scene `walls-e10-ruins.tile.py`, demo entry `wall-ruins` | pending Shawn |
| E32 | **Stone slab flooring** (`walls/floor.py: StoneFloor`, Shawn: "floor tiles — full 35mm or whatever slabs gridding the ground"): the masonry unit kernel laid horizontal — one `_block_mesh` slab per DB square (`slabs_per_square` subdivides), joints on the grid lines with the SOIL showing through (dirt between pavers), per-slab seat on its own terrain patch (+ tiny tilt) so pavement rolls with the ground, `missing_prob` dirt patches + `spall_prob` broken corners, wall texture presets shared.  Slab footprints stamp `terrain_support_z` + `obstacle_mask`.  Scene `walls-e11-floor.tile.py`, demo entry `floor-stone`, production tiles `src/tiles/floors/` | Shawn: two fixes → E33 |
| E33 | **The floor IS the terrain** (Shawn: slabs "right to the bottom of the terrain"; "None of the floor terrain should be on top of soil.  The floor IS the terrain, replaces the soil"): every paved pitch cell (missing-slab cells included) has its terrain LOWERED to a thin dirt bed (`bed_mm` 1.2); slabs run FULL DEPTH from the tile bottom (z = 0) to the pavement level (`top_mm` 4.2 ± 0.3) — no soil at pavement level and no undercut, by construction; joints read as deep dark grid slots, missing slabs as sunken dirt pits.  Plus cracked slabs: the standard rock crack promoted to `stone/cracks.py: engrave_cracks` (shared by scatter stones + floors; the last private cross-package import gone), walk length scaled to the slab (`n_segs` param, rock default unchanged), `crack_prob` 0.20 (demo 0.6) | Shawn: two more calls → E35 |
| E35 | **Official wall heights + floor drybrush** (Shawn: "fix the wall heights to match the official visible height" + "Dry brush!  Dry brush!"): `height_mm` default 49.7 → 29.6 — the official pieces put their wall TOP 33.1 mm above the datum (49.7 was the piece bbox including the 16.6 tall base); scenes drop their explicit heights.  Floor slabs get the shared aged-relief pass at slab scale; the first two rounds read as CORDUROY — a handful of plane waves forms directional interference trains on a large flat face (curvature hides this on rocks) — fixed by subdividing first and widening `aged_relief` with `broad_waves`/`grain_waves` overrides (40/24 for floors; rock defaults 6/16 unchanged): isotropic granular tooth + broad undulation | Shawn: REJECTED — reads as chop/fabric in MeshLab → E36 |
| E36 | **The common stone relief** (Shawn: "rethink rock texture — find some reference images, and build a new common rock texture that can be applied across all rock/block types"): references gathered (docs/reference/stone-texture/) + the OFFICIAL floors measured by ray-cast height grids — the stone read is a CALM PLATEAU CARVED DOWNWARD (TS-019: RMS 0.32, skew −0.99, no sub-2 mm power); design doc docs/design/stone-surface-texture.md.  New primitives: `stone/noise.py` (isotropic value-noise fBm — NO plane waves, no phase to cohere into corduroy) and `stone/finish.py: stone_relief` (−carve·smoothstep(fbm) saturating worn floors + gentle dish + curvature damp), statistically matched to the officials (ours: RMS 0.35, skew −0.84).  Wired: floor slabs (defaults; carve faded at the base) and `_block_mesh` (carve = 2×relief_mm capped at 0.12×min dimension, softer band 0.55 — the first mapping ATE the worn blocks and left drip-ribs at brick bases).  Fieldstone + scatter rocks still on aged_relief — migration is the explicit next step.  Also: `separate_pinches` now stretches DEGENERATE CONNECTED pairs (<20 µm) too — manifold unions emit sliver edges at tangencies that STL reload collapsed into count-4 edges | pending Shawn |
| E60 | **Regular running bond** (Shawn: "a wall variant where the bricks are all the same size, regular runs rather than irregular — another variant within the same engine") — the `bond='running'` layout preset the family taxonomy always anticipated (build order #2, "not new machinery"): `_running_heights` lays UNIFORM courses and `_running_cuts` lays uniform-length bricks on a grid phase-shifted a half-brick on alternate courses (end bricks become closers; a cut within `_CLOSER_MIN` 1.5 mm of an end is dropped so no sliver forms).  `RegularBrickWall(CutStoneWall)` in brick.py pairs it with a near-flush dressed unit (yaw 0.25°, tilt 0.15°, face-recess 0.12 — dead-level courses) and the brick-scale rowlock surround; no spall/missing (use `ruin=` for a broken regular wall).  Openings + the bond-fit flow around a doorway unchanged.  Scene `walls-e17-regular-brick.tile.py` (regular front / worn back, side by side), watertight.  `bond` defaults `'irregular'` → every existing wall bit-identical | pending Shawn |
